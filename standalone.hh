...
class RedisClient<RCT, std::enable_if_t<(RCT == RedisClientType::STANDALONE)>>
  : public seastar::enable_shared_from_this<
      RedisClient<RCT, std::enable_if_t<(RCT == RedisClientType::STANDALONE)>>>,
...
#define MakeSharedEnablerClass                                                 \
  class make_shared_enabler                                                    \
    : public RedisClient<RedisClientType::STANDALONE> {                        \
   public:                                                                     \
    make_shared_enabler(Args &&... args)                                       \
      : RedisClient<RedisClientType::STANDALONE>(                              \
  ...
  static future<> GetClientImplFunc(T instance_ptr) {
    return instance_ptr->UpdateConnection()
      .then([instance_ptr] { return instance_ptr->FetchRoleMachingNode(); })
      .then([instance_ptr =
               std::move(instance_ptr)](std::optional<Node> &&hp_pair_opt) {
        if (hp_pair_opt.has_value()) {
          auto &&hp_pair = hp_pair_opt.value();
          instance_ptr->available_hosts_ports_.push_back(hp_pair);
          return instance_ptr->UpdateConnection(hp_pair.first, hp_pair.second);
        }
  ...
  static future<std::shared_ptr<RedisClient<RedisClientType::STANDALONE>>>
  GetRedisClient(Args &&... args) {
    MakeSharedEnablerClass;
    std::shared_ptr<RedisClient<RedisClientType::STANDALONE>> instance_ptr =
  ...
  template <typename... Args>
  static future<seastar::shared_ptr<RedisClient<RedisClientType::STANDALONE>>>
  GetSeastarRedisClient(Args &&... args) {
    MakeSharedEnablerClass;
    seastar::shared_ptr<RedisClient<RedisClientType::STANDALONE>> instance_ptr =
            ...
            .finally([this] {
              if (IsUpdateFail() && client_params_.auto_reconnect) {
                return UpdateConnection()
                  .then([this] { return FetchRoleMachingNode(); })
                  .then([this](std::optional<Node> &&hp_pair_opt) {
                    if (hp_pair_opt.has_value()) {
                      auto &&hp_pair = hp_pair_opt.value();
                      this->available_hosts_ports_.emplace_back(hp_pair);
                      return UpdateConnection(hp_pair.first, hp_pair.second);
                    }
                    return make_ready_future();
                  });
              } else {
...
#undef ConsumeWithCallbackImpl
 ...
 private:
  inline bool IsUpdateFail() {
    return (!this->standalone_connection_) ||
