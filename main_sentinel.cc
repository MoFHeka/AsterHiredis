...
class APP {
 public:
  ...
  seastar::foreign_ptr<
    seastar::shared_ptr<RedisClient<RedisClientType::SENTINEL>>>
    instance_ptr;

 private:
  future<seastar::shared_ptr<RedisClient<RedisClientType::SENTINEL>>>
  CreateInstance() {
    auto client_params = ClientParams();
    client_params.redis_instance_type = RedisClientType::SENTINEL;
    client_params.hosts_and_ports = Node("127.0.0.1", 26379);
    client_params.redis_master_name = "mymaster";
    client_params.redis_role = RedisRole::RANDOM;
    return RedisClient<RedisClientType::SENTINEL>::GetSeastarRedisClient(
      client_params);
  }
 ...
 public:
  future<RedisReplyVectorSPtr> set_ok_test() {
    ...
    return do_with(
             std::move(client_instance_ptr),
             [](auto ptr) {
               return ptr->Send("set key val").then([ptr = std::move(ptr)] {
                 return ptr->Recv().then(
  ...
  future<> set_test() {
    ...
    return do_with(std::move(client_instance_ptr),
                   [](auto ptr) { return ptr->Send("set key val"); })
...
int main(int argc = 0, char **argv = NULL) {
  ...
  const auto timeout_ms = std::chrono::milliseconds(1000000 * 10) + time_start;
  ...
  auto set_ok_test =
    seastar::alien::submit_to(app_->alien(), 0, [app_engine] {
