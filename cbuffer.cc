...
static void __create_buffer_mirror(cbuf_t *cb) {
  ...
  cb->data =
    mmap(NULL, cb->size << 1, PROT_NONE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
  if (cb->data == MAP_FAILED) fail();

  address = mmap(cb->data, cb->size, PROT_READ | PROT_WRITE,
                 MAP_FIXED | MAP_SHARED, fd, 0);
  if (address != cb->data) fail();

  address = mmap(cb->data + cb->size, cb->size, PROT_READ | PROT_WRITE,
                 MAP_FIXED | MAP_SHARED, fd, 0);
  if (address != cb->data + cb->size) fail();

  status = close(fd);
  if (status) fail();
}

cbuf_t *cbuf_new(const uint32_t order) {
...
int cbuf_is_empty(const cbuf_t *me) { return me->head == me->tail; }
...
int cbuf_size(const cbuf_t *me) { return me->size; }
...
int cbuf_unusedspace(const cbuf_t *me) { return me->size - cbuf_usedspace(me); }
