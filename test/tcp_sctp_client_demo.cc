 ...
 public:
  class connection {
   ...
   public:
    ...
    future<> do_read() {
      return read_buf_.read_exactly(rx_msg_size)
    ...
    future<> do_write(int end) {
      if (end == 0) { return make_ready_future(); }
      return write_buf_.write(str_txbuf)
        .then([this] {
          _bytes_write += tx_msg_size;
          return write_buf_.flush();
        })
        .then([this, end] { return do_write(end - 1); });
    }

    future<> ping(int times) {
      return write_buf_.write("ping")
        .then([this] { return write_buf_.flush(); })
        .then([this, times] {
          return read_buf_.read_exactly(4).then(
            [this, times](temporary_buffer<char> buf) {
              if (buf.size() != 4) {
    ...
    future<size_t> rxrx() {
      return write_buf_.write("rxrx")
        .then([this] { return write_buf_.flush(); })
        .then([this] {
          return do_write(tx_msg_nr)
            .then([this] { return write_buf_.close(); })
            .then([this] { return make_ready_future<size_t>(_bytes_write); });
        });
    }

    future<size_t> txtx() {
      return write_buf_.write("txtx")
        .then([this] { return write_buf_.flush(); })
        .then([this] {
          return do_read().then(
            [this] { return make_ready_future<size_t>(_bytes_read); });
        });
    }
  };
  ...
  future<> rxrx_test(connection *conn) {
    ...
    return conn->rxrx().then([started](size_t bytes) {
      auto finished = lowres_clock::now();
      (void)clients.invoke_on(0, &client::rxtx_report, started, finished,
                              bytes);
    });
  }
  ...
  void ping_report(lowres_clock::time_point started,
  ...
  void rxtx_report(lowres_clock::time_point started,
                   lowres_clock::time_point finished, size_t bytes) {
    if (_earliest_started > started) _earliest_started = started;
    if (_latest_finished < finished) _latest_finished = finished;
    ...
    if (++_num_reported == _concurrent_connections) {
      ...
      fmt::print(std::cout, "Bandwidth(Gbits/Sec): {}\n",
                 static_cast<double>((_processed_bytes * 8)) /
                   (1000 * 1000 * 1000) / secs);
      (void)clients.stop().then([] { engine().exit(0); });
    }
  }
  ...
  future<> start(ipv4_addr server_addr, std::string test, unsigned ncon) {
    ...
    for (unsigned i = 0; i < ncon; i++) {
      ...
      (void)connect(make_ipv4_address(server_addr), local, protocol)
...
int main(int ac, char **av) {
  app_template app;
  app.add_options()("server", bpo::value<std::string>()->required(),
                    "Server address")(
    "test", bpo::value<std::string>()->default_value("ping"),
    "test type(ping | rxrx | txtx)")("conn",
                                     bpo::value<unsigned>()->default_value(16),
                                     "nr connections per cpu")(
    "proto", bpo::value<std::string>()->default_value("tcp"),
  ...
  return app.run_deprecated(ac, av, [&app] {
    ...
    (void)clients.start().then([server, test, ncon]() {
      return clients.invoke_on_all(&client::start, ipv4_addr{server}, test,
                                   ncon);
    });
  });
}
