...
class httpclient_ {
 ...
 public:
  httpclient_(unsigned duration, unsigned total_conn, unsigned reqs_per_conn)
    : _duration(duration), _conn_per_core(total_conn / smp::count),
      _reqs_per_conn(reqs_per_conn), _run_timer([this] { _timer_done = true; }),
  ...
  class connection {
   private:
    ...
    httpclient_ *_httpclient_;
    uint64_t _nr_done{0};

   public:
    connection(connected_socket &&fd, httpclient_ *client)
      : fd_(std::move(fd)), read_buf_(fd_.input()), write_buf_(fd_.output()),
        _httpclient_(client) {}

    uint64_t nr_done() { return _nr_done; }

    future<> do_req() {
      return write_buf_.write("GET / HTTP/1.1\r\nHost: 127.0.0.1:10000\r\n\r\n")
        .then([this] { return write_buf_.flush(); })
        .then([this] {
          _parser.init();
          return read_buf_.consume(_parser).then([this] {
            // Read HTTP response header first
            if (_parser.eof()) { return make_ready_future<>(); }
            ...
            return read_buf_.read_exactly(content_len)
              .then([this](temporary_buffer<char> buf) {
                _nr_done++;
                http_debug("%s\n", buf.get());
                if (_httpclient_->done(_nr_done)) {
                  return make_ready_future();
                } else {
                  return do_req();
                }
  ...
  future<> connect(ipv4_addr server_addr) {
    ...
    for (unsigned i = 0; i < _conn_per_core; i++) {
      ...
      (void)seastar::connect(make_ipv4_address(server_addr))
  ...
  future<> run() {
    ...
    if (_timer_based) { _run_timer.arm(std::chrono::seconds(_duration)); }
...
int main(int ac, char **av) {
  ...
  app.add_options()(
    "server,s",
    bpo::value<std::string>()->default_value("192.168.66.100:10000"),
    "Server address")("conn,c", bpo::value<unsigned>()->default_value(100),
                      "total connections")(
    "reqs,r", bpo::value<unsigned>()->default_value(0), "reqs per connection")(
    "duration,d", bpo::value<unsigned>()->default_value(10),
  ...
  return app.run(ac, av, [&app]() -> future<int> {
    ...
    return httpclient_s
      ->start(std::move(duration), std::move(total_conn),
              std::move(reqs_per_conn))
      .then([httpclient_s, server] {
        return httpclient_s->invoke_on_all(&httpclient_::connect,
                                           ipv4_addr{server});
      })
      .then([httpclient_s] {
        return httpclient_s->invoke_on_all(&httpclient_::run);
      })
      .then([httpclient_s] {
        return httpclient_s->map_reduce(adder<uint64_t>(),
                                        &httpclient_::total_reqs);
      })
      .then([httpclient_s, started](auto total_reqs) {
        ...
        fmt::print("==========     done     ============\n");
        return httpclient_s->stop().then([httpclient_s] {
