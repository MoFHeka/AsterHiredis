...
class RedisClient<RCT, std::enable_if_t<(RCT == RedisClientType::CLUSTER)>>
  : public seastar::enable_shared_from_this<
      RedisClient<RCT, std::enable_if_t<(RCT == RedisClientType::CLUSTER)>>>,
    public RedisClientBase {

 protected:
  Shards slot_range_map_;
  NodeMap cluster_connections_;
  Node main_connection_host_port_;
  ConnectionSPtr main_connection_;
  std::deque<Node> available_hosts_ports_;

 protected:
  future<> CommonActionAfterConnect(ConnectionSPtr conn);
...
#define MakeSharedEnablerClass                                                 \
  class make_shared_enabler : public RedisClient<RedisClientType::CLUSTER> {   \
   public:                                                                     \
    make_shared_enabler(Args &&... args)                                       \
      : RedisClient<RedisClientType::CLUSTER>(std::forward<Args>(args)...) {}  \
  };
  ...
  static future<std::shared_ptr<RedisClient<RedisClientType::CLUSTER>>>
  GetRedisClient(Args &&... args) {
    MakeSharedEnablerClass;
    std::shared_ptr<RedisClient<RedisClientType::CLUSTER>> instance_ptr =
  ...
  template <typename... Args>
  static future<seastar::shared_ptr<RedisClient<RedisClientType::CLUSTER>>>
  GetSeastarRedisClient(Args &&... args) {
    MakeSharedEnablerClass;
    seastar::shared_ptr<RedisClient<RedisClientType::CLUSTER>> instance_ptr =
...
#undef MakeSharedEnablerClass
      ...
      if (network_stack_error &&
        ...
        (void)smp::submit_to(this_shard_id(), [this] {
          return seastar::sleep(client_params_.redis_wait_timeout *
            ...
            .then([this] {
              for (const auto &node_map_ : this->cluster_connections_) {
                if (!node_map_.second->connection_gate_.is_closed()) {
                  return node_map_.second->connection_gate_.close();
                }
              }
 ...
 public:
  ...
  virtual future<> Send(const Node &node, const char *source) override {
    auto &&conn = this->cluster_connections_[node];
    return this->connection_capacity_
      .wait(
        ConnectionUnit::TimeoutEndMilliSec(client_params_.redis_wait_timeout),
        1)
      .then([this, conn, source] { return conn->Send(source); })
  ...
  virtual future<> Send(const Node &node, int argc, const char **argv,
    ...
    return this->connection_capacity_
      .wait(
        ConnectionUnit::TimeoutEndMilliSec(client_params_.redis_wait_timeout),
        1)
      .then([this, conn, argc, argv, argvlen] {
        return conn->Send(argc, argv, argvlen);
      })
      .handle_exception([this](auto ep) {
        connection_send_ex_ = ep;
        return HandleCommandException<>(ep);
  ...
  virtual future<> Send(const std::string_view &guide_key,
                        const char *source) override {
    auto slot = shards::GetSlot(guide_key);
    auto node = shards::GetNode(slot, this->slot_range_map_);
    return this->Send(node, source);
  }

  virtual future<> Send(const std::string_view &guide_key, int argc,
                        const char **argv, const size_t *argvlen) override {
    auto slot = shards::GetSlot(guide_key);
    auto node = shards::GetNode(slot, this->slot_range_map_);
    return this->Send(node, argc, argv, argvlen);
  }

  template <typename... Args>
  future<> Send(const Node &node, const Args &... args) {
    auto &&conn = this->cluster_connections_[node];
    return this->connection_capacity_
      .wait(
        ConnectionUnit::TimeoutEndMilliSec(client_params_.redis_wait_timeout),
      ...
      .then([this, conn,
             args =
               std::make_tuple(std::forward<const Args &>(args)...)]() mutable {
        return std::apply(  // std::apply compatible with C++17 to capture pack
          [this, conn](auto &&... args) { return conn->Send(args...); },
          std::move(args));
      })
      .handle_exception([this](auto ep) {
        connection_send_ex_ = ep;
  ...
  template <typename... Args>
  future<> Send(const std::string_view &guide_key, const Args &... args) {
    auto slot = shards::GetSlot(guide_key);
    auto node = shards::GetNode(slot, this->slot_range_map_);
    return this->Send(node, std::forward<const Args &>(args)...);
  }

  virtual future<> WritePipeLine(const Node &node,
                                 const char *source) override {
    auto &&conn = this->cluster_connections_[node];
    return this->connection_capacity_
      .wait(
        ConnectionUnit::TimeoutEndMilliSec(client_params_.redis_wait_timeout),
        1)
      .then([this, conn, source] { return conn->WritePipeLine(source); })
      .handle_exception([this](auto ep) {
        connection_send_ex_ = ep;
        return HandleCommandException<>(ep);
      })
  ...
  virtual future<> WritePipeLine(const Node &node, int argc, const char **argv,
    ...
    return this->connection_capacity_
      .wait(
        ...
        1)
      .then([this, conn, argc, argv, argvlen] {
        return conn->WritePipeLine(argc, argv, argvlen);
      })
      .handle_exception([this](auto ep) {
        connection_send_ex_ = ep;
        return HandleCommandException<>(ep);
  ...
  virtual future<> WritePipeLine(const std::string_view &guide_key,
    ...
    auto slot = shards::GetSlot(guide_key);
    auto node = shards::GetNode(slot, this->slot_range_map_);
    return this->WritePipeLine(node, source);
  }
  ...
  virtual future<> WritePipeLine(const std::string_view &guide_key, int argc,
    ...
    auto slot = shards::GetSlot(guide_key);
    auto node = shards::GetNode(slot, this->slot_range_map_);
    return this->WritePipeLine(node, argc, argv, argvlen);
  }

  template <typename... Args>
  future<> WritePipeLine(const Node &node, const Args &... args) {
    auto &&conn = this->cluster_connections_[node];
    return this->connection_capacity_
      .wait(
        ConnectionUnit::TimeoutEndMilliSec(client_params_.redis_wait_timeout),
      ...
      .then([this, conn,
        ...
        return std::apply(  // std::apply compatible with C++17 to capture pack
          [this, conn](auto &&... args) {
            return conn->WritePipeLine(args...);
          },
          std::move(args));
      })
      .handle_exception([this](auto ep) {
  ...
  template <typename... Args>
  future<> WritePipeLine(const std::string_view &guide_key,
                         const Args &... args) {
    auto slot = shards::GetSlot(guide_key);
    auto node = shards::GetNode(slot, this->slot_range_map_);
    return this->WritePipeLine(node, std::forward<const Args &>(args)...);
  }

  virtual future<> SendPipeLine(const Node &node) override {
    auto &&conn = this->cluster_connections_[node];
    return this->connection_capacity_
      .wait(
        ConnectionUnit::TimeoutEndMilliSec(client_params_.redis_wait_timeout),
        1)
      .then([this, conn] { return conn->SendPipeLine(); })
      .handle_exception([this](auto ep) {
        connection_send_ex_ = ep;
        return HandleCommandException<>(ep);
      })
  ...
  virtual future<> SendPipeLine(const std::string_view &guide_key) override {
    auto slot = shards::GetSlot(guide_key);
    auto node = shards::GetNode(slot, this->slot_range_map_);
    return this->SendPipeLine(node);
  }

  virtual future<RedisReplyVectorSPtr> Recv(const Node &node) override {
    auto &&conn = this->cluster_connections_[node];
    return this->connection_capacity_
      .wait(
        ConnectionUnit::TimeoutEndMilliSec(client_params_.redis_wait_timeout),
        1)
      .then([this, conn] { return conn->Recv(); })
  ...
  virtual future<RedisReplyVectorSPtr>
  Recv(const std::string_view &guide_key) override {
    auto slot = shards::GetSlot(guide_key);
    auto node = shards::GetNode(slot, this->slot_range_map_);
    return this->Recv(node);
  }

#define ConsumeWithCallbackImpl                                                \
  auto &&conn = this->cluster_connections_[node];                              \
  return do_with(                                                              \
    std::move(input_consumer), [this, conn](auto &input_consumer) {            \
      return this->connection_capacity_.wait(1)                                \
        .then([this, conn, &input_consumer] {                                  \
          return conn->ConsumeWithCallback(input_consumer);                    \
        })                                                                     \
        .handle_exception([this](auto ep) {                                    \
  ...
  // Keeping handle input stream by ConsumeCallback until reaching eof.
  template <class S>
  future<> ConsumeWithCallback(const Node &node, const S input_consumer) {
    ConsumeWithCallbackImpl
  }
  template <class S>
  future<> ConsumeWithCallback(const std::string_view &guide_key,
                               const S input_consumer) {
    auto slot = shards::GetSlot(guide_key);
    auto node = shards::GetNode(slot, this->slot_range_map_);
    return this->ConsumeWithCallback(node, std::move(input_consumer));
  }
  ...
  virtual future<> ConsumeWithCallback(
    const std::string_view &guide_key,
    const InputStreamConsumerFunction input_consumer) override {
    auto slot = shards::GetSlot(guide_key);
    auto node = shards::GetNode(slot, this->slot_range_map_);
    return this->ConsumeWithCallback(node, std::move(input_consumer));
...
#undef ConsumeWithCallbackImpl
 ...
 private:
  future<> CreateInitialConnection(const Node &pair);
  future<ConnectionSPtr> CreateConnectionAndTest(const std::string &host,
                                                 const uint16_t port);
  future<> CreateClusterConnection();

  inline bool IsUpdateFail() {
    return (!this->main_connection_) ||
