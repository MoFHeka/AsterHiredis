...
class APP {
 public:
  ...
  seastar::foreign_ptr<
    seastar::shared_ptr<RedisClient<RedisClientType::CLUSTER>>>
    instance_ptr;
 ...
 public:
  future<RedisReplyVectorSPtr> set_ok_test() {
    ...
    return do_with(
             ...
             [](auto ptr) {
               return ptr->Send("key", "set key val").then([ptr = std::move(ptr)] {
                 return ptr->Recv("key").then(
                   [ptr = std::move(ptr)](RedisReplyVectorSPtr rptr) {
                     for (auto reply_it = rptr->begin();
                          reply_it != rptr->end(); ++reply_it) {
                       if ((*reply_it)->type == REDIS_REPLY_STRING || (*reply_it)->type == REDIS_REPLY_STATUS) {
