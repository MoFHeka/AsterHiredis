...
class APP {
 public:
  ...
  seastar::foreign_ptr<
    seastar::shared_ptr<RedisClient<RedisClientType::STANDALONE>>>
    instance_ptr;

 private:
  future<seastar::shared_ptr<RedisClient<RedisClientType::STANDALONE>>>
  CreateInstance() {
    auto client_params = ClientParams();
    client_params.hosts_and_ports = Node("127.0.0.1", 6379);
    client_params.redis_role = RedisRole::RANDOM;
    return RedisClient<RedisClientType::STANDALONE>::GetSeastarRedisClient(
      client_params);
  }
